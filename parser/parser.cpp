/***************************************************************************
    ...
    CurvePlot Formula Parser

    Original parser:
	Mark C. Peterson (Fractint)

    Subsequently extended by:
	Chuck Ebbert
	Tim Wegner
	Paul de Leeuw

    This parser converts mathematical expressions into a compact
    executable stack program which is evaluated for each plotted point.
...
extern	INT_PTR CALLBACK ErrorDlg(HWND, UINT, WPARAM, LPARAM);
...
int	    debugflag, col, row;
//struct	    DComplex New, old;
int	    save_release;
int	    overflow;
int	    bitshift;
...
char	ErrorMessage[360];

int pstopmsg(int x,char *msg)
    {
    wsprintf(ErrorMessage, "%s, ErrorType = %d", msg, x);
    DialogBox((HINSTANCE)GetWindowLongPtr(GlobalHwnd, GWLP_HINSTANCE), "ErrorDlg", GlobalHwnd, ErrorDlg);
...
static	int StoPtr, LodPtr, OpPtr;      /* static CAE fp */
...
_CMPLX ComplexPower(_CMPLX x, _CMPLX y) 
    {
    _CMPLX z, cLog, t = {0,0};
   ...
   */
   ...
   for(i = 0; *JumpList[i]; i++)
      if(strlen(JumpList[i]) == (WORD)Len)
         if(!_strnicmp(JumpList[i], Str, Len))
            return i + 1;
   return 0;
}
   ...
   if(Str[Len+n] == '(') {
      for(n = 0; n < sizeof(FnctList) / sizeof(struct FNCT_LIST); n++) {
         if(strlen(FnctList[n].s) == (WORD)Len) {        /* TIW 03-31-91 added far */
            if(!_strnicmp(FnctList[n].s, Str, Len)) {  /* TIW 03-31-91 added far */
...
int ParseStr(char *Str, int pass) {
...
// Added to take in z PHD 2014-04-05
   v[3].a.d.x = z.x;
   v[3].a.d.y = 0.0;
...
int frmgetconstant(char * FrmData, struct token_st * tok)
{
   int c;
   int i = 1;
   int getting_base = 1;
//   long filepos = ftell(openfile);
   ptrdiff_t  filepos = StrPtr - FrmData;
...
void is_complex_constant(char * FrmData, struct token_st * tok)
{
   ...
   while (!done) {
      switch (c = frmgetchar(FrmData)) {
         ...
         case '-' :
            ...
            sign_value = -1;
            if ((c = frmgetchar(FrmData)) == '.' || isdigit(c)) {
               if (debug_token != NULL) {
                  fprintf(debug_token,  "Set temp_tok.token_str[0] to %c\n", c);
               }
               temp_tok.token_str[0] = (char) c;
      ...
      if (!done && frmgetconstant (FrmData, &temp_tok)) {
         c = frmgetchar(FrmData);
         if (debug_token != NULL) {
...
int frmgetalpha(char * FrmData, struct token_st * tok)
{
   int c;
   int i = 1;
...
//   long last_filepos = ftell(openfile);
   ptrdiff_t  last_filepos = StrPtr - FrmData;
   while ((c=frmgetchar(FrmData)) != EOF && c != '\032') {
      filepos = StrPtr - FrmData;
//      filepos = ftell(openfile);
      switch (c) {
...
void frm_get_eos (char * FrmData, struct token_st * this_token)
{
//   long last_filepos = ftell(openfile);
    ptrdiff_t  last_filepos = StrPtr - FrmData;
   int c;
   while ((c = frmgetchar(FrmData)) == '\n' || c == ',' || c == ':') {
      if (c == ':') {
    this_token->token_str[0] = ':';
      }
//      last_filepos = ftell(openfile);
      last_filepos = StrPtr - FrmData;
   }
   if (c == '}') {
      this_token->token_str[0] = '}';
...
int frmgettoken(char * FrmData, struct token_st * this_token)
{
   int c;
   int i=1;
   ptrdiff_t  filepos;
   ...
   switch (c = frmgetchar(FrmData)) {
      CASE_NUM: case '.':
         ...
         return frmgetconstant(FrmData, this_token);
      CASE_ALPHA: case '_':
         this_token->token_str[0] = (char) c;
         return frmgetalpha(FrmData, this_token);
      CASE_TERMINATOR:
...
//               fseek(openfile, filepos, SEEK_SET);
		StrPtr = FrmData + filepos;
            ...
            this_token->token_id   = 0;
             ...
             || this_token->token_str[0] == ','
             || this_token->token_str[0] == ':' ) {
            frm_get_eos(FrmData, this_token);
         }
         else if (this_token->token_str[0] == ')') {
            ...
            this_token->token_id = CLOSE_PARENS;
              ...
              /* the following function will set token_type to PARENS and
                 ...
                 complex constant */
            is_complex_constant(FrmData, this_token);
               return 1;
         }
...
void frm_error(char * FrmData, ptrdiff_t begin_frm)
{
   struct token_st tok;
/* char debugmsg[500];
...
//      fseek(open_file, begin_frm, SEEK_SET);
      StrPtr = FrmData + begin_frm;
      line_number = 1;
//      while (ftell(open_file) != errors[j].error_pos) {
//         if((i = fgetc(open_file)) == '\n') {
      while (StrPtr - FrmData != errors[j].error_pos) {
         if((i = *StrPtr++) == '\n') {
            line_number++;
         }
         else if (i == '\0' || i == '}') {
...
//            fseek (open_file, errors[j].error_pos, SEEK_SET);
            StrPtr = FrmData + errors[j].error_pos;
            frmgettoken(FrmData, &tok); // reset file to end of error token 
            return;
         }
      }
      ...
      i = (int)strlen(msgbuf);
      StrPtr = FrmData + errors[j].start_pos;
      statement_len = token_count = 0;
      done = 0;
      while(!done) {
         if(errors[j].error_pos == StrPtr - FrmData)  {
/*          stopmsg(0, "About to get error token\n");
*/          chars_to_error = statement_len;
            frmgettoken(FrmData, &tok);
...
int frm_prescan (char * FrmData)
{
    ptrdiff_t  filepos;
   int i;
   ptrdiff_t  statement_pos, orig_pos;
   int done = 0;
...
//    char msgbuf[80] = "Just got ";
      filepos = StrPtr - FrmData;
      frmgettoken (FrmData, &this_token);
