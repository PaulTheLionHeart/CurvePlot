/*
    ...
    Creates a CF_DIB bitmap from the current DIB and places it
...
BYTE *GetOrthoPalette(BYTE *buffer)
	...
	for(g=0;g<8;++g) 
		...
		palette[i*RGB_SIZE+RGB_RED] = rr;
		palette[i*RGB_SIZE+RGB_GREEN] = gg;
		palette[i*RGB_SIZE+RGB_BLUE] = bb;
		}
	    }
	}
...
BYTE *CreateDib(WORD dx, WORD dy, BYTE *palette, WORD bits)
    ...
    if (palette != NULL) 
	{
	pRgb = (RGBQUAD *)((BYTE *)lpbi + (unsigned int)lpbi->biSize);
